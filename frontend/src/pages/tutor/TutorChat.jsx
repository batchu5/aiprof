...
export const TutorChat = () => {
  ...
  return (
    <div className="flex h-[calc(100vh-5.5rem)] overflow-hidden bg-slate-950 text-slate-100 rounded-2xl border border-slate-800 shadow-2xl relative">
      {/* LEFT SIDEBAR - Desktop */}
      <div
        className={`${
          sidebarOpen ? 'w-72' : 'w-0 hidden md:flex md:w-0'
        } transition-all duration-300 ease-in-out bg-slate-900/90 border-r border-slate-800 flex-col shrink-0 relative overflow-hidden`}
      >
        <div className="p-4 border-b border-slate-800/80 flex items-center justify-between">
          <button
            onClick={handleCreateNewConversation}
            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-indigo-600 via-purple-600 to-indigo-700 hover:from-indigo-500 hover:to-purple-500 text-white rounded-xl text-sm font-semibold shadow-lg shadow-indigo-500/20 transition-all active:scale-[0.98]"
          >
      ...
      {mobileDrawerOpen && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm md:hidden flex">
          <div className="w-72 bg-slate-900 h-full border-r border-slate-800 flex flex-col p-4">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800">
              <span className="font-bold text-sm text-slate-200">Conversations</span>
            ...
            <button
              onClick={handleCreateNewConversation}
              className="mt-4 w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-semibold shadow-md"
            >
              <Plus className="w-4 h-4" /> New Conversation
            </button>
              ...
              {conversations.map((conv) => (
                <div
                  ...
                  className={`p-3 rounded-xl text-xs font-medium cursor-pointer ${
                    conv.id === activeConvId ? 'bg-indigo-600/30 text-indigo-300' : 'bg-slate-800/50 text-slate-300'
                  }`}
                >
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-slate-950/40 relative">
        ...
        <div className="px-4 py-3.5 border-b border-slate-800/80 bg-slate-900/60 backdrop-blur-md flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-sm font-bold text-slate-100 flex items-center gap-2 truncate">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 custom-scrollbar">
          {messagesLoading ? (
            ...
            <div className="flex flex-col items-center justify-center min-h-[70%] text-center p-6 space-y-6 max-w-xl mx-auto">
              <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-indigo-500/20 to-purple-500/20 border border-indigo-500/30 flex items-center justify-center text-indigo-400 shadow-xl shadow-indigo-500/10">
                <BookOpen className="w-8 h-8" />
              </div>

              <div>
                <h2 className="text-xl font-bold text-slate-100">Ask me anything about your learning materials! 🎓</h2>
              ...
              <div className="w-full space-y-2 pt-2">
                ...
                {starterQuestions.map((q, idx) => (
                  <button
                    ...
                    className="w-full text-left p-3 rounded-xl bg-slate-900/80 hover:bg-indigo-600/10 border border-slate-800 hover:border-indigo-500/40 text-slate-300 hover:text-indigo-200 text-xs transition-all flex items-center justify-between group"
                  >
                    <span>{q}</span>
                    <Sparkles className="w-3.5 h-3.5 text-slate-400 group-hover:text-indigo-400 transition-colors" />
                  </button>
                ))}
              </div>
            </div>
                    ...
                    <div
                      className={`rounded-2xl px-5 py-4 text-sm shadow-lg leading-relaxed ${
                        isUser
                          ...
                          : 'bg-slate-900/90 text-slate-200 border border-slate-800 rounded-tl-none backdrop-blur-sm'
                      }`}
                    >
                      {isUser ? (
                        <p className="whitespace-pre-wrap">{msg.content}</p>
                      ) : (
                        <div className="prose prose-invert max-w-none text-xs sm:text-sm prose-p:leading-relaxed prose-headings:text-slate-100 prose-strong:text-indigo-300 prose-code:text-purple-300">
          ...
          {loading && (
            <div className="flex items-start gap-3">
              ...
              <div className="bg-slate-900/90 text-slate-300 rounded-2xl px-5 py-4 border border-slate-800 flex items-center gap-2">
                <span className="text-xs text-slate-400 font-medium">AI Tutor is searching materials</span>
        ...
        <div className="p-4 border-t border-slate-800/80 bg-slate-900/80 backdrop-blur-md shrink-0">
          <form
            onSubmit={(e) => {
              e.preventDefault();
              handleSendMessage();
          ...
          >
            <div className="relative flex items-end gap-2 bg-slate-950 border border-slate-800 focus-within:border-indigo-500 rounded-2xl p-2 transition-all">
              <textarea
                ref={textareaRef}
                value={input}
                ...
                rows={1}
                disabled={loading}
                className="w-full bg-transparent text-slate-100 placeholder-slate-500 text-xs sm:text-sm px-3 py-2 focus:outline-none resize-none min-h-[40px] max-h-[120px] custom-scrollbar"
              />

              <button
                type="submit"
                disabled={!input.trim() || loading}
                className="p-2.5 rounded-xl bg-gradient-to-tr from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-md shadow-indigo-500/20 shrink-0"
              >
      ...
      {selectedSource && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-cyan-800/60 rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4">
            ...
            <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 text-xs text-slate-300 leading-relaxed max-h-60 overflow-y-auto custom-scrollbar">
