    ...
    <div className="space-y-8 animate-fade-in max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Comprehension performance and study telemetry</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        <div className="p-4 rounded-2xl bg-slate-900/70 border border-slate-800 flex items-center gap-3.5">
          ...
          <div>
            <p className="text-[11px] font-semibold text-slate-400 uppercase">Spaces</p>
            <p className="text-xl font-bold text-slate-100">{overall.total_spaces ?? 0}</p>
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-slate-900/70 border border-slate-800 flex items-center gap-3.5">
          ...
          <div>
            <p className="text-[11px] font-semibold text-slate-400 uppercase">Projects</p>
            <p className="text-xl font-bold text-slate-100">{overall.total_projects ?? 0}</p>
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-slate-900/70 border border-slate-800 flex items-center gap-3.5">
          ...
          <div>
            <p className="text-[11px] font-semibold text-slate-400 uppercase">Materials</p>
            <p className="text-xl font-bold text-slate-100">{overall.total_materials ?? 0}</p>
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-slate-900/70 border border-slate-800 flex items-center gap-3.5">
          ...
          <div>
            <p className="text-[11px] font-semibold text-slate-400 uppercase">Active Days</p>
            <p className="text-xl font-bold text-slate-100">{overall.active_days ?? 0}</p>
          </div>
        </div>

        <div className="p-4 rounded-2xl bg-slate-900/70 border border-slate-800 flex items-center gap-3.5 col-span-2 sm:col-span-1">
          <div className="p-2.5 rounded-xl bg-amber-500/10 text-amber-400 border border-amber-500/20">
            <Flame className="w-5 h-5" />
          </div>
          <div>
            <p className="text-[11px] font-semibold text-slate-400 uppercase">Current Streak</p>
          ...
          <div className="flex flex-col items-center justify-center py-6 space-y-4">
            <div className="relative w-36 h-36 flex items-center justify-center">
              <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="42" className="text-slate-800" strokeWidth="10" stroke="currentColor" fill="transparent" />
                <circle
                  cx="50"
                  cy="50"
                  r="42"
              ...
              <div className="absolute flex flex-col items-center justify-center text-center">
                <span className="text-3xl font-extrabold text-white">{Math.round(perf.overall_mastery || 0)}%</span>
          ...
          <Card title="Concept Distribution Breakdown" subtitle="Distribution across mastery states">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 my-4">
              <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-center">
                ...
                <p className="text-2xl font-extrabold text-white mt-1">{perf.concepts_mastered ?? 0}</p>
              </div>

              <div className="p-4 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-center">
                <p className="text-xs text-indigo-400 font-bold uppercase">Improving (50-79%)</p>
                <p className="text-2xl font-extrabold text-white mt-1">{perf.concepts_improving ?? 0}</p>
              </div>

              <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-center">
                <p className="text-xs text-amber-400 font-bold uppercase">Needs Attention (&lt;50%)</p>
                <p className="text-2xl font-extrabold text-white mt-1">{perf.concepts_needing_attention ?? 0}</p>
              </div>
            </div>
              ...
              <div className="w-full h-3 bg-slate-950 rounded-full overflow-hidden flex border border-slate-800">
                {totalConcepts > 0 ? (
                  <>
                    ...
                    <div className="h-full bg-indigo-500" style={{ width: `${((perf.concepts_improving || 0) / totalConcepts) * 100}%` }} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Card title="Top Performing Projects" subtitle="Ranked by overall comprehension mastery">
          {topProjects.length > 0 ? (
                ...
                <div key={idx} className="p-3.5 rounded-xl bg-slate-900/60 border border-slate-800 flex items-center justify-between text-xs">
                  <div className="flex items-center gap-3">
                    <span className={`w-6 h-6 rounded-lg flex items-center justify-center font-bold text-xs ${
                      idx === 0 ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30' : 'bg-slate-800 text-slate-400'
                    }`}>
                      #{idx + 1}
                    </span>
                    <div>
                      <p className="font-bold text-slate-100">{proj.name}</p>
                      <p className="text-[11px] text-slate-400 mt-0.5">{proj.space_name}</p>
                    </div>
                  </div>
            ...
            <div className="py-10 text-center bg-slate-900/40 border border-slate-800/80 rounded-xl my-2">
              <FolderOpen className="w-8 h-8 text-indigo-400 mx-auto mb-2 opacity-60" />
              <p className="text-sm font-semibold text-slate-200">No Analytics Data Available Yet</p>
              <p className="text-xs text-slate-400 mt-1">Create study projects and complete quizzes to track project performance.</p>
            </div>
          )}
        </Card>
        ...
        <Card title="Priority Areas to Improve" subtitle="Target concepts requiring additional focus">
          {areasToImprove.length > 0 ? (
              ...
              {areasToImprove.map((item, idx) => (
                <div key={idx} className="p-3.5 rounded-xl bg-amber-500/5 border border-amber-500/20 flex items-center justify-between text-xs">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-slate-100">{item.concept}</p>
                      <p className="text-[11px] text-slate-400 mt-0.5">Project: {item.project}</p>
                    </div>
                  </div>
            ...
            <div className="py-10 text-center bg-slate-900/40 border border-slate-800/80 rounded-xl my-2">
              ...
              <p className="text-sm font-semibold text-slate-200">No Weak Areas Identified</p>
              <p className="text-xs text-slate-400 mt-1">Start studying or take quizzes to identify concepts needing target review.</p>
            </div>
          )}
        </Card>
      </div>
      ...
      <Card title="AI Telemetry & Feature Usage" subtitle="Usage distribution across AI tutor, quiz authoring, and evaluation">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 my-4">
          <div className="p-4 rounded-xl bg-slate-900/80 border border-slate-800 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-slate-400">Tutor Questions Asked</p>
              <p className="text-xl font-bold text-slate-100">{aiUsage.total_tutor_questions ?? 0}</p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-xl bg-slate-900/80 border border-slate-800 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-slate-100">{aiUsage.total_quizzes ?? 0}</p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-xl bg-slate-900/80 border border-slate-800 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-slate-400">AI Feedbacks Processed</p>
              <p className="text-xl font-bold text-slate-100">{aiUsage.total_ai_feedback ?? 0}</p>
            </div>
          </div>
        </div>
